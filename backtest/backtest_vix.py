...
vix_lower_bound = 10
vix_upper_bound = 15
trading_fee = 0.003  # 수수료 0.15%

ticker_list = ["SPY", "^VIX", "^VIX1D", "^VIX3M", "SVXY", "UVIX", "SVIX"]
...
# 포지션 신호 (1: 보유, 0: 현금)
data["sig_UVIX"] = np.where((data["^VIX1D_prior"] <= vix_lower_bound) & (data["^VIX_prior"] < data["^VIX3M_prior"]), 1, 0)
data["sig_svix"] = np.where((data["^VIX1D_prior"] >= vix_upper_bound) & (data["^VIX_prior"] < data["^VIX3M_prior"]), 1, 0)

# 현재 어떤 종목을 보유 중인지 표시 (0: 현금, 1: UVIX, 2: SVIX)
data["current_pos"] = 0
data.loc[data["sig_UVIX"] == 1, "current_pos"] = 1
...
# 1) 순수 매매 손익 (수수료 제외)
data["raw_pnl"] = (data["sig_UVIX"] * data["UVIX_ret"] * capital_per_trade) + \
...
final_list = ["strat_ideal", "UVIX", "SVIX", "SPY"]
